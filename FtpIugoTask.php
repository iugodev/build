 ...
 * 
 * Example usage:
 * <ftpiugo host="host" port="21" username="user" password="password" dir="public_html" mode="ascii" clearfirst="true" depends="false" filemode="" dirmode="">
 *   <fileset dir=".">
 *     <include name="**"/>
 *     <exclude name="phing"/>
 ...
 * </ftpiugo>
 */
class FtpIugoTask extends Task {
    ...
    public function main() {
        ...
        $ret = $ftp->connect();
        if(@PEAR::isError($ret)) {
            throw new BuildException('NO SE PUEDE CONECTAR AL SERVIDOR DE FTP '.$this->host.' EN EL PUERTO '.$this->port.': '.$ret->getMessage());
        } else {
            $this->log('CONECTADO AL SERVIDOR FTP ' . $this->host . ' EN EL PUERTO ' . $this->port, $this->logLevel);
        }

        $ret = $ftp->login($this->username, $this->password);
        if(@PEAR::isError($ret)) {
            throw new BuildException('NO SE PUEDE LOGUEAR AL SERVIDOR DE FTP '.$this->host.' EN EL PUERTO '.$this->port.' CON EL USUARIO '.$this->username.': '.$ret->getMessage());
        } else {
            $this->log('LOGUEADO AL SERVIDOR FTP CON EL USUARIO ' . $this->username, $this->logLevel);
        }

        if ($this->passive) {
            $this->log('SETEADO MODO PASIVO', $this->logLevel);
            $ret = $ftp->setPassive();
            if(@PEAR::isError($ret)) {
                $ftp->disconnect();
                throw new BuildException('NO SE PUEDE SETEAR EL MODO PASIVO: '.$ret->getMessage());
            }
        }
...
';
        ...
        if($this->clearFirst) {
            ...
            $this->log('SE BORRO TODO EN '.$dir, $this->logLevel);
            $ftp->rm($dir, true);
        }

        if($this->clearCache && !$this->clearFirst){
            // Clear cache
            $this->log('CACHE BORRADO '.$dir.'tmp/cache/', $this->logLevel);
            $ftp->rm($dir.'tmp/cache/', true);
            $ftp->mkdir($dir.'tmp/cache',true);
        }
        ...
        if(@PEAR::isError($ret)) {
            ...
            $this->log('MOVIENDOSE AL DIRECTORIO '.$dir, $this->logLevel);
        }
        
        $fs = FileSystem::getFileSystem();
        $convert = $fs->getSeparator() == '\\';

        echo 'BUSCANDO ARCHIVOS Y CARPETAS PARA SUBIR
';
        ...
        foreach($this->filesets as $fs) {
            ...
            foreach($srcDirs as $dirname) {
                ...
                if(!$this->_directoryInformations($ftp, $remoteFileInformations, $dirname)){
                    $this->log('NUEVA CARPETA '.$dirname, $this->logLevel);
                    echo '--> '.$dirname.'
';
                    $ret = $ftp->mkdir($dirname, true);
                    if(@PEAR::isError($ret)) {
                        $ftp->disconnect();
                        throw new BuildException('NO SE PUDO CREAR EL DIRECTORIO '.$dirname.': '.$ret->getMessage());
                    }
                }
                if($this->dirmode){
            ...
            foreach($srcFiles as $filename) {
                ...
                if(!$this->depends || ($local_filemtime > $remoteFileModificationTime) ){
                    $this->log($file->getCanonicalPath() .' SUBIDO A '.$filename, $this->logLevel);
                    echo '--> '.$filename.'
';
                    $ret = $ftp->put($file->getCanonicalPath(), $filename, true, $this->mode);
                    if(@PEAR::isError($ret)) {
                        $ftp->disconnect();
                        throw new BuildException('NO SE PUDO SUBIR EL ARCHIVO '.$filename.': '.$ret->getMessage());
                    }
                }
                if($this->filemode){
        ...
        $ftp->disconnect();
        $this->log('DESCONECTADO DEL SERVIDOR FTP', $this->logLevel);
        echo 'FINALIZO LA COMPROBACION
';
    }
